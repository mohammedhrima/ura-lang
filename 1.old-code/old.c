...
#define ERR_ARG_TYPE_MISMATCH "Argument %d type mismatch in call to '%s'"

#define ERR_ARG_NEEDS_REF     "Argument %d to '%s' must be passed by reference (ref x)"

#define ERR_ARG_NO_REF        "Argument %d to '%s' does not take a reference"

#define ERR_REF_TYPE_MISMATCH "Reference type mismatch: expected %s, got %s"

#define ERR_BITWISE_NEEDS_INT "Bitwise and shift operators require integer operands"

#define ERR_NOT_AN_ARRAY      "Cannot index '%s', it is not an array"

#define ERR_INDEX_NOT_INT     "Array index must be an integer, got %s"

#define ERR_BY_NEEDS_RANGE    "'by' sets the step of a range (a..b), so it needs one on its left"

#define ERR_BY_NOT_INT        "The 'by' step must be an integer, got %s"
...
#define ERR_FOR_NOT_ITERABLE      "'for %s in ...' expects a range (a..b) or an array"

#define ERR_FOR_REF_NEEDS_ARRAY   "'for ref' needs an array; a range yields values, not storage"

#define ERR_RETURN_TYPE_MISMATCH  "'%s' returns %s, but this returns %s"

#define ERR_RETURN_NEEDS_REF      "'%s' returns a reference; return `ref x` (or null), not a value"

#define ERR_RETURN_NO_REF         "'%s' returns a value, not a reference; drop the `ref`"

#define ERR_CANNOT_CAST           "Cannot cast %s to %s"

#define ERR_LEN_NOT_ARRAY         "'.len' is only valid on an array, not %s"

#define ERR_UNTERM_BLOCK_COMMENT  "Unterminated block comment, expected '*/'"

#define ERR_UNTERM_STRING_LITERAL "Unterminated string literal, expected '\"'"

#define ERR_UNTERM_CHAR_LITERAL   "Unterminated character literal, expected \"'\""
...
#define ERR_UNTERM_USE_PATH        "Unterminated 'use' path, expected closing '\"'"

#define ERR_UNTERM_LINK_PATH       "Unterminated 'link' path, expected closing '\"'"
...
#define ERR_FN_EXPECTED_PARAM_NAME "Expected parameter name in function %s"

#define ERR_FN_EXPECTED_RET_TYPE   "Expected <data type> after function %s"
...
#define ERR_STRUCT_EMPTY "Struct %s must declare at least one field"
...
#define ERR_ENUM_EMPTY          "Enum %s must declare at least one variant"

#define ERR_ENUM_VALUE_INT      "Enum variant '%s' needs an integer value after '='"
...
#define ERR_NEW_EXPECTED_ARRAY_TYPE "Expected an array type after 'new' (e.g. new int[n])"

#define ERR_DROP_NO_PARAMS          "Destructor 'drop' takes no parameters; write 'operator drop:'"

#define ERR_DROP_NO_RET             "Destructor 'drop' returns void; drop the return type"

#define ERR_ASSIGN_OPTIONAL                           \
    "'%s' may be null; unwrap it with '?? <default>'" \
    " or declare '%s' as optional"

#define ERR_FALLBACK_RHS        "The right side of '?\?' may itself be null; it must not be"

#define ERR_FALLBACK_LHS        "The left side of '?\?' is never null, so '?\?' does nothing"

#define ERR_NULL_NO_TARGET      "'%s' can never be null; declare it optional to assign null to it"

#define ERR_COMPARE_NON_NULL    "'%s' is never null, so this comparison is always the same"

#define ERR_NULL_ON_VALUE       "Only arrays and pointers can be null, not %s"

#define ERR_REDECL_FUNCTION     "Redeclaration of function '%s'"

#define ERR_SIG_CONFLICT        "Conflicting declaration of '%s': %s"

#define NOTE_PREV_DECL          "Previous declaration of '%s' is here"

#define ERR_UNDECLARED_VARIABLE "Undeclared variable '%s'"

#define ERR_BARE_FIELD          "Field '%s' must be accessed through self; write 'self.%s'"

#define ERR_REF_TO_NON_VARIABLE "Cannot take a reference to a non-variable"
...
#define ERR_REF_NEEDS_VARIABLE "A reference must be bound to a variable (ref x)"

#define ERR_REF_FIELD_WRITE    "'%s' is a reference field; bind it with '= ref', not a value"

#define ERR_THROW_NEEDS_ERROR  "'throw' expects an Error, got %s"

#define ERR_MOD_EXPECTED_COLON "Expected ':' to open the body of module '%s'"

#define ERR_MOD_EMPTY          "Module '%s' is empty; add a declaration or remove it"

#define ERR_MOD_BODY           "Only 'fn', 'struct', 'enum' and 'mod' can be declared in a module"

#define ERR_UNKNOWN_MODULE     "Unknown module '%s'"

#define ERR_MOD_NO_MEMBER      "Module '%s' has no member '%s'"

#define ERR_MOD_DOT_CALL       "'%s' is a module; reach its members with '::' instead of '.'"
...
#define ERR_BINOP_TYPE_MISMATCH "Cannot use '%s' with %s and %s"

#define ERR_NO_OPERATOR                         \
    "Struct %s has no 'operator %s' taking %s;" \
    " declare one inside the struct"

#define ERR_ARG_TYPE_MISMATCH_STRUCT "Cannot assign %s to %s; they are different structs"

#define ERR_OPERATOR_OUTSIDE_STRUCT  "'operator' may only be declared inside a struct"

#define ERR_PUB_OUTSIDE_STRUCT "'pub' marks a static method; it may only be used inside a struct"
...
#define ERR_METHOD_ON_NON_STRUCT "Cannot call '.%s()' on %s; only a struct has methods"
...
#define ERR_CLEAN_NEEDS_ARRAY "'clean' frees a heap array, not %s"
...
#define TODO(cond, fmt, ...)                                \
    if (_check(FILE, FUNC, LINE, cond, fmt, ##__VA_ARGS__)) \
        exit(1);
#define debug(fmt, ...)   \
    if (ura.enable_debug) \
...
#define DATA_TYPES                                                                               \
    SIGNED_TYPES, UNSIGNED_TYPES, F32, F64, BOOL, CHAR, CHARS, VOID, PTR, ARRAY_TYPE, LIST_TYPE, \
        FN_TYPE
...
#define ASSIGNS_OP                                                                       \
    ADD_ASSIGN, SUB_ASSIGN, MUL_ASSIGN, DIV_ASSIGN, MOD_ASSIGN, BAND_ASSIGN, BOR_ASSIGN, \
...
#define PASSES_Os \
    "default<Os>" // Optimize for small binary, like O2 + limiting the size of the executable
...
/* ---- LLVM C API calls older versions lack ---- */
#if LLVM_VERSION_MAJOR < 9
// the debug location API took and returned the metadata wrapped as a value
static MetadataRef LLVMGetCurrentDebugLocation2(Builder builder) {
    Value loc = LLVMGetCurrentDebugLocation(builder);
    return loc ? LLVMValueAsMetadata(loc) : NULL;
}

static void LLVMSetCurrentDebugLocation2(Builder builder, MetadataRef loc) {
    LLVMSetCurrentDebugLocation(builder, loc ? LLVMMetadataAsValue(ura.context, loc) : NULL);
}
#endif

#if LLVM_VERSION_MAJOR < 8
// no explicitly typed builder calls yet: pointers are typed, so the type a
// pointer points to is the type these calls would have been given
static Value LLVMBuildLoad2(Builder b, TypeRef type, Value ptr, const char *name) {
    (void)type;
    return LLVMBuildLoad(b, ptr, name);
}

static Value LLVMBuildGEP2(Builder b, TypeRef type, Value ptr, Value *idx, unsigned n,
                           const char *name) {
    (void)type;
    return LLVMBuildGEP(b, ptr, idx, n, name);
}

static Value LLVMBuildStructGEP2(Builder b, TypeRef type, Value ptr, unsigned idx,
                                 const char *name) {
    (void)type;
    return LLVMBuildStructGEP(b, ptr, idx, name);
}

static Value LLVMBuildCall2(Builder b, TypeRef type, Value fn, Value *args, unsigned n,
                            const char *name) {
    (void)type;
    return LLVMBuildCall(b, fn, args, n, name);
}

static Value LLVMBuildIntCast2(Builder b, Value v, TypeRef type, LLVMBool is_signed,
                               const char *name) {
    unsigned from = LLVMGetIntTypeWidth(LLVMTypeOf(v)), to = LLVMGetIntTypeWidth(type);
    if (from < to)
        return is_signed ? LLVMBuildSExt(b, v, type, name) : LLVMBuildZExt(b, v, type, name);
    if (from > to)
        return LLVMBuildTrunc(b, v, type, name);
    return v;
}

// call the memset intrinsic directly; since LLVM 7 its alignment is an
// attribute on the destination instead of an argument
static Value LLVMBuildMemSet(Builder b, Value ptr, Value val, Value len, unsigned align) {
    TypeRef i8p = LLVMPointerType(ura.i8, 0);
    TypeRef params[4] = { i8p, ura.i8, ura.i64, ura.i1 };
    Value fn = LLVMGetNamedFunction(ura.module, "llvm.memset.p0i8.i64");
    if (!fn)
        fn = LLVMAddFunction(ura.module, "llvm.memset.p0i8.i64",
                             LLVMFunctionType(ura.vd, params, 4, 0));
    Value args[4] = {
        LLVMBuildPointerCast(b, ptr, i8p, ""),
        val,
        LLVMBuildZExtOrBitCast(b, len, ura.i64, ""),
        LLVMConstInt(ura.i1, 0, 0),
    };
    Value call = LLVMBuildCall(b, fn, args, 4, "");
    if (align) {
        unsigned kind = LLVMGetEnumAttributeKindForName("align", 5);
        LLVMAddCallSiteAttribute(call, 1, LLVMCreateEnumAttribute(ura.context, kind, align));
    }
    return call;
}
#endif
...
void emit_out_array(Token *field, Value slot, Value frame, int depth, int fd);
    ...
    for (int i = 0; i < ura.sources_count; i++)
        if (strcmp(ura.sources[i]->pathname, full) == 0) {
            if (ura.sources[i]->loading)
                parse_warn(NULL,
                           "import cycle: '%s' imports itself through"
                           " '%s'",
                           file_name, ura.current->filename);
            free(full);
            return;
        }
...
char *to_string(Type type) {
    char *res[END + 1] = {
        ...
        [BOR_ASSIGN] = "OR_ASS",
        [BXOR_ASSIGN] = "XOR_ASS",
        [LSHIFT_ASSIGN] = "SHL_ASS",
        [RSHIFT_ASSIGN] = "SHR_ASS",
        [MOD] = "MOD",
        [COMA] = "COMA",
        [ACCESS] = "ACC",
        [REF] = "REF",
...
char *type_name(Type type) {
    ...
    case FN_TYPE:
        return "fn";
    default:
        return type ? to_string(type) : "unknown";
    }
}
...
int _vprint(File out, const char *conv, va_list args) {
    ...
    for (int i = 0; conv[i]; i++) {
        ...
        if (strncmp(conv + i, "zu", 2) == 0) {
            ...
            switch (conv[i]) {
            case 'c':
                res += fprintf(out, "%c", va_arg(args, int));
                break;
            case 's': {
            ...
            case 'p':
                res += fprintf(out, "%p", va_arg(args, void *));
                break;
            case 'x': {
                unsigned int value = va_arg(args, unsigned int);
                res += (prec >= 0) ? fprintf(out, "%.*x", prec, value) : fprintf(out, "%x", value);
                break;
            }
            case 'X': {
                unsigned int value = va_arg(args, unsigned int);
            ...
            case 'd': {
                ...
                res += (prec >= 0) ? fprintf(out, "%.*d", prec, value) : fprintf(out, "%d", value);
                break;
            }
            case 'f': {
                double value = va_arg(args, double);
                res += (prec >= 0) ? fprintf(out, "%.*f", prec, value) : fprintf(out, "%f", value);
                break;
            }
            case '%':
                res += fprintf(out, "%%");
            ...
            case 'k': {
                ...
                case I64: {
                    ...
                    switch (token->type) {
                    case I32:
                        fprintf(out, "[%lld] ", (long long)token->Int.value);
                        break;
                    case I64:
                        fprintf(out, "[%lld] ", token->Long.value);
                        break;
                    case BOOL: {
                    ...
                    case F32:
                        fprintf(out, "[%f] ", token->Float.value);
                        break;
                    case CHAR: {
                        fprintf(out, "[");
                ...
                if (token->is_ref)
                    fprintf(out, "ref ");
                if (token->ret_type)
                    fprintf(out, "ret [%s] ", to_string(token->ret_type));
                if (token->is_variadic)
                    fprintf(out, "variadic ");
                break;
            }
            default:
                TODO(1, "invalid format specifier [%c]", conv[i]);
                break;
            }
        }
    }
...
void print_node_label(Node *node) {
    ...
    switch (token->type) {
    case I32:
        printf("int %lld", (long long)token->Int.value);
        return;
    case I64:
        printf("long %lld", token->Long.value);
        return;
    case I16:
        printf("short %d", token->Short.value);
        return;
    case F32:
        printf("float %g", token->Float.value);
        return;
    case BOOL:
        printf("bool %s", token->Bool.value ? "True" : "False");
        return;
    case CHAR: {
        printf("char '");
    ...
    case STRUCT_DEF:
        printf("struct %s", token->name);
        return;
    case MODULE:
        printf("mod %s", token->name);
        return;
    case NEW:
        printf("new %s", token->Struct.name);
        return;
    case DOT:
        printf(".%s", token->name);
        break;
    default:
        printf("%s", spell(token->type));
    }
...
void pnode(Node *node, char *indent) {
    if (!node || !node->token || !ura.enable_debug)
        return;
    Node **subs = NULL;
    int subs_count = 0;
    int subs_size = 0;
...
#define push(n)                     \
    ...
    for (int i = 0; i < subs_count; i++) {
        Node *child = subs[i];
        if (!child || !child->token || !child->token->type)
            continue;
...
void render_caret(File out, Token *token, const char *color) {
    if (!token || !token->source || !token->source->content)
        return;
...
bool platform_has(char *name) {
    for (int i = 0; ura.platform && ura.platform[i]; i++)
        if (strcmp(ura.platform[i], name) == 0)
            return true;
    return false;
}
...
bool lex_identifier(char *src, int *i, int line, int indent, int base) {
    ...
    if (lex_use(src, i, s, line)) {
        if (ura.error_count || ura.sources_count == loaded)
            return true;
        ura.calling_use++;
        tokenize(indent);
        ura.calling_use--;
...
bool lex_symbol(char *src, int *i, int line, int *indent) {
    ...
    for (size_t j = 0; j < sizeof(specials) / sizeof(*specials); j++) {
        ...
        if (strncmp(specials[j].name, src + (*i), len) == 0) {
            ...
            if (includes(specials[j].type, DOTS, 0))
                (*indent) += TAB;
            return true;
        }
    }
...
Token *parse_token(int line, int s, int e, Type type, int indent) {
    ...
    case ID: {
        ...
        static const Keyword keywords[] = {
            ...
            { "sizeof", SIZEOF, 0, 1 },
            { "List", LIST_TYPE, 0, 1 },
            { "array", ARRAY_TYPE, 0, 1 },
            { "null", NULL_LIT, 0, 1 },
        };

        for (size_t i = 0; i < sizeof(keywords) / sizeof(*keywords); i++) {
            if (strcmp(new->name, keywords[i].name) != 0)
                continue;
...
Token *next() {
    Token *token = peek(0);
    if (ura.exe_pos + 1 < ura.tokens_count)
        ura.exe_pos++;
    return token;
}
...
Token *peek(int index) {
    ...
    if (pos >= ura.tokens_count)
        pos = ura.tokens_count - 1;
...
void parser_recover(int indent) {
    ...
    while (ura.exe_pos < ura.tokens_count && ura.tokens[ura.exe_pos]->type != END) {
        ...
        if (t->indent <= indent && t->line != start_line)
            break;
        ura.exe_pos++;
    }
...
void qualify_decl(Node *n, char *prefix) {
    ...
    set_name(t, q);
    free(q);
    if (!includes(t->type, STRUCT_DEF, ENUM_DEF, MODULE, 0))
        return;
    for (int i = 0; i < n->children_count; i++) {
        if (t->type == STRUCT_DEF && n->children[i]->token->type != FN_DEC)
            continue;
...
Node *prime_node() {
    ...
    case OPERATOR: {
        ...
        Token *op = find(ASSIGN, ADD, SUB, MUL, DIV, MOD, EQUAL, NOT_EQUAL, LESS, GREAT, LESS_EQUAL,
                         GREAT_EQUAL, ADD_ASSIGN, SUB_ASSIGN, MUL_ASSIGN, DIV_ASSIGN, MOD_ASSIGN,
                         BAND, BOR, BXOR, LSHIFT, RSHIFT, BAND_ASSIGN, BOR_ASSIGN, BXOR_ASSIGN,
                         LSHIFT_ASSIGN, RSHIFT_ASSIGN, 0);
        if (!op) {
        ...
        if (fn->Fn.params_count > 1) {
            ...
            char *sub = rhs->ret_type == STRUCT_CALL ? rhs->Struct.name : type_name(rhs->ret_type);
    ...
    case MODULE: {
        ...
        for (int i = 0; i < node->children_count; i++) {
            ...
            if (!includes(ct->type, FN_DEC, STRUCT_DEF, ENUM_DEF, MODULE, 0)) {
    ...
    case ID: {
        ...
        bool fn_type = peek(0)->type == FN_DEC && peek(1)->type == LPAR;
    ...
    case REF: {
        ...
        bool fn_type = peek(1)->type == FN_DEC && peek(2)->type == LPAR;
...
Node *expr_node(int min_op) {
    ...
    while (true) {
        static const int res[END + 1] = {
            [ASSIGN] = 1,      [ADD_ASSIGN] = 1,    [SUB_ASSIGN] = 1,    [MUL_ASSIGN] = 1,
            [DIV_ASSIGN] = 1,  [MOD_ASSIGN] = 1,    [BAND_ASSIGN] = 1,   [BOR_ASSIGN] = 1,
            [BXOR_ASSIGN] = 1, [LSHIFT_ASSIGN] = 1, [RSHIFT_ASSIGN] = 1, [FALLBACK] = 2,
            [OR] = 2,          [AND] = 3,           [RANGE] = 2,         [BOR] = 4,
            [BXOR] = 5,        [BAND] = 6,          [LESS] = 8,          [EQUAL] = 7,
            [NOT_EQUAL] = 7,   [GREAT] = 8,         [LESS_EQUAL] = 8,    [GREAT_EQUAL] = 8,
            [ADD] = 10,        [LSHIFT] = 9,        [RSHIFT] = 9,        [SUB] = 10,
            [MUL] = 11,        [DIV] = 11,          [AS] = 12,           [MOD] = 11,
        };
...
void exit_scope() {
    if (CHECK(ura.scopes_count < 0, "No active scope to exit"))
        return;
    ...
    for (int i = ura.scopes_count; i >= 0; i--) {
        ...
        for (int j = 0; j < scope->variables_count; j++) {
            ...
            if (!current || strcmp(current, name) != 0)
                continue;
            bool from_outer_fn = passed_function && scope->token->type == FN_DEC;
            if (captured)
                *captured = from_outer_fn;
            if (from_field)
                *from_field = scope->token->type == STRUCT_DEF;
            return scope->variables[j];
        }
        if (scope->token->type == FN_DEC)
...
char *signature_diff(Token *old, Token *new) {
    if (old->Fn.params_count != new->Fn.params_count)
        return format("expected %d parameter(s), found %d", old->Fn.params_count,
                      new->Fn.params_count);
    ...
    if (!same_or_pointer(old->ret_type, new->ret_type))
        return format("returns %s, expected %s", type_name(new->ret_type),
                      type_name(old->ret_type));
    return format("one is variadic, the other is not");
}

bool same_signature(Token *a, Token *b) {
    if (!same_or_pointer(a->ret_type, b->ret_type))
        return false;
    if (a->is_variadic != b->is_variadic)
        return false;
    if (a->Fn.params_count != b->Fn.params_count)
        return false;
...
Node *find_module_in(Node *parent, char *name) {
    for (int i = 0; i < parent->children_count; i++) {
        ...
        if (c->token->type != MODULE)
            continue;
        if (strcmp(c->token->name, name) == 0)
            return c;
...
bool rewrite_struct_ctor(Node *node) {
    Node *base = node;
    int depth = 0;
    while (base->token->type == ACCESS) {
        base = base->left;
        depth++;
    }
    Token *name = base->token;
    if (name->type != ID || name->is_dec)
        return false;
    if (find_variable(name->name, NULL, NULL))
        return false;
...
Node *find_method(Node *def, char *name) {
    for (int i = 0; i < def->children_count; i++) {
        ...
        if (child->token->type != FN_DEC)
            continue;
        if (strcmp(child->token->name, name) == 0)
            return child;
    }
    return NULL;
}
...
void analyze(Node *node) {
    ...
    case FN_DEC: {
        ...
        for (int i = 0; i < node->children_count; i++)
            if (node->children[i]->token->type == FN_DEC)
    ...
    case STRUCT_DEF: {
        if (token->has_drop) {
            ...
            for (int i = 0; i < node->children_count && !found; i++) {
                Token *child = node->children[i]->token;
                if (child->type != FN_DEC || !child->name)
                    continue;
        ...
        for (int i = 0; i < node->children_count; i++) {
            ...
            if (field->token->type == STRUCT_DEF)
                declare_struct(field);
            analyze(field);
        }
        exit_scope();
        break;
    ...
    case ID: {
        ...
        if (decl) {
            ...
            if (decl->ret_type == FN_TYPE)
                token->Fn = decl->Fn;
            if (decl->ret_type == ARRAY_TYPE)
                token->Array = decl->Array;
    ...
    case FOR: {
        ...
        if (it->type == RANGE)
            ...
            if (it->Array.depth > 1)
                var->ret_type = ARRAY_TYPE;
            else
                var->ret_type = it->Array.sub_type;
    ...
    case TYPEOF:
    case SIZEOF:
        if (!is_data_type(node->left->token))
            analyze(node->left);
        break;
    case ARRAY: {
        resolve_struct_type(token);
...
TypeRef to_llvm_type(Type type) {
    ...
    case VOID:
        return ura.vd;
    case PTR:
        return LLVMPointerType(ura.i8, 0);
    default:
        TODO(1, "to_llvm_type: unhandled type %t", type);
        return NULL;
    }
}
...
TypeRef array_type(Token *arr, int depth) {
    TypeRef fields[2] = { LLVMPointerType(elem_type(arr, depth), 0), ura.i64 };
...
TypeRef struct_type_of(Node *def) {
    ...
    if (token->llvm.struct_type)
        return token->llvm.struct_type;
    ...
    for (int i = 0; i < def->children_count; i++) {
        ...
        if (field->type == STRUCT_DEF) {
            struct_type_of(child);
            continue;
        }
        if (!is_field(field))
            continue;
        TypeRef t = llvm_type_of(field);
        fields[field->Struct.index] = field->is_ref ? LLVMPointerType(t, 0) : t;
        n++;
    }
...
TypeRef llvm_type_of(Token *token) {
    ...
    for (int i = 0; i < n; i++)
        params[i] = llvm_type_of(token->Fn.params[i]);
...
void type_check_fcall(Node *node) {
    ...
    if (token->is_static_call && !token->Fcall.ptr) {
        ...
        if (mf) {
            ...
            type_check_fcall(node);
            if (token->ret_type == STRUCT_CALL)
                token->Struct = mf->token->Struct;
            return;
        }
        Node *def = find_struct(token->Struct.name);
        if (!def) {
            if (find_module(token->Struct.name))
                parse_error(token, ERR_MOD_NO_MEMBER, token->Struct.name, token->name);
            else
                parse_error(token, ERR_UNKNOWN_TYPE, token->Struct.name);
            return;
        }
    ...
    bool bad_count =
        fn->is_variadic ? node->children_count < wanted : node->children_count != wanted;
    if (bad_count) {
    ...
    if (ret->ret_type == ARRAY_TYPE)
        token->Array = ret->Array;
}
...
bool struct_contains(Node *def, Node *target, int depth) {
    ...
    for (int i = 0; i < def->children_count; i++) {
        ...
        if (!is_field(field) || field->is_ref)
            continue;
        ...
        if (sub == target)
            return true;
        if (struct_contains(sub, target, depth + 1))
            return true;
    }
    return false;
}
...
bool is_field(Token *token) {
    return token->name && token->type != STRUCT_DEF && token->type != FN_DEC;
}
...
Node *operator_named(Node *node, bool as_ref) {
    ...
    if (!def)
        return NULL;
    char *q = format("%s.%s.%s%s", def->token->name, spell(node->token->type), as_ref ? "ref." : "",
...
bool find_operator(Node *node) {
    ...
    if (!fn)
        fn = operator_named(node, !spelled);
    if (!fn)
        return false;
    token->Fcall.ptr = fn;
    token->ret_type = fn->token->ret_type;
...
bool is_untyped_literal(Node *node) {
    if (!node)
        return false;
    if (includes(node->token->type, I32, F32, 0))
        return true;
    if (!includes(node->token->type, MATH_TYPE, 0))
        return false;
...
bool literal_value(Node *node, long *out) {
    ...
    long rhs;
    if (node->token->type == SUB && node->left && node->right && node->left->token->type == I32 &&
        node->left->token->Int.value == 0 && literal_value(node->right, &rhs)) {
        *out = -rhs;
        return true;
    }
    return false;
...
void check_int_range(Node *node, Type target) {
    ...
    if (!literal_value(node, &value))
        return;
    if (!int_range(target, &lo, &hi))
        return;
    if (value >= lo && value <= hi)
        return;
    parse_error(node->token, ERR_LITERAL_OUT_OF_RANGE, (long long)value, type_name(target),
                (long long)lo, (long long)hi);
}

void retype_literal(Node *node, Type target) {
    if (!is_untyped_literal(node))
        return;
...
void adopt_literal(Node *node, Type target) {
    ...
    if (b == BOOL)
        ...
        fam = includes(a, NUMERIC_TYPES, 0) && includes(b, NUMERIC_TYPES, 0);
    if (!fam)
        return;
    if (!is_float(a))
        check_int_range(node, target);
...
void unify_literals(Node *left, Node *right) {
    ...
    if (is_untyped_literal(left) && is_untyped_literal(right) && int_range(lt, &llo, &lhi) &&
        int_range(rt, &rlo, &rhi)) {
...
void type_check(Node *node) {
    ...
    case RETURN: {
        ...
        if (!is_assignable(ura.fn_ret, value)) {
            parse_error(node->token, ERR_ASSIGN_OPTIONAL, value->name ? value->name : "the value",
                        ura.fn_ret->name);
            return;
        }
        Type got = node->left->token->ret_type;
        if (got && got != wanted)
            parse_error(node->token, ERR_RETURN_TYPE_MISMATCH, ura.fn_ret->name, type_name(wanted),
                        type_name(got));
        break;
    }
    case FN_CALL:
    ...
    case AS: {
        ...
        if (bad_src || bad_dst)
            parse_error(token, ERR_CANNOT_CAST, struct_name_of(node->left->token),
    ...
    case LOOP: {
        ...
        if (node->left && node->left->token->ret_type != BOOL)
            parse_error(node->token, "The '%s' condition must be a bool, got %s", node->token->name,
                        type_name(node->left->token->ret_type));
        for (int i = 0; i < node->children_count; i++)
            type_check(node->children[i]);
        type_check(node->right);
        break;
            ...
            if (branch->left)
                for (int j = 0; j < branch->left->children_count; j++) {
                    ...
                    if (subject && ret_type && ret_type != subject)
                        parse_error(value->token, ERR_CASE_TYPE_MISMATCH, type_name(ret_type),
                                    type_name(subject));
                }
            for (int j = 0; j < branch->children_count; j++)
    ...
    case THROW: {
        ...
        if (!is_err)
            parse_error(node->token, ERR_THROW_NEEDS_ERROR, type_name(v->ret_type));
        break;
    }
    case ARRAY_LIT: {
    ...
    case SIZEOF:
        ...
        if (token->type == TYPEOF)
            set_string_type(node->token);
        else
            node->token->ret_type = U64;
        break;
    case CLEAN: {
        type_check(node->left);
        Token *target = node->left->token;
        node->token->ret_type = VOID;
        if (target->ret_type == ARRAY_TYPE)
            break;
        if (target->ret_type == STRUCT_CALL && target->is_ref)
            break;
    ...
    case RSHIFT: {
        ...
        if (token->type == ASSIGN && !is_assignable(lhs, rhs)) {
            if (is_null(rhs))
                parse_error(token, ERR_NULL_NO_TARGET, lhs->name);
            else
                parse_error(token, ERR_ASSIGN_OPTIONAL, rhs->name, lhs->name);
            return;
        }
        ...
        if (lt == STRUCT_CALL) {
            ...
            bool binding = assign && node->left->token->is_ref && node->right->token->type == REF;
            if (!is_init && !binding && find_operator(node))
                return;
        ...
        if (lt && rt && lt != rt) {
            if (token->type == ASSIGN)
                ...
                parse_error(token, ERR_BINOP_TYPE_MISMATCH, spell(token->type),
                            struct_name_of(node->left->token), struct_name_of(node->right->token));
            return;
        }
...
void init_module(char *name) {
    ...
    ura.debug_file =
        LLVMDIBuilderCreateFile(ura.debug_builder, file, strlen(file), ura.dir, strlen(ura.dir));
    ura.debug_compile_unit = LLVMDIBuilderCreateCompileUnit(
        ...
        LLVMDWARFEmissionFull, 0, 0, 0, "", 0, "", 0); // LLVM 11 added sysroot and SDK
#else
...
// LLVM 13 added the new pass manager's C API and LLVM 17 removed the legacy one
void run_passes(char *passes) {
#if LLVM_VERSION_MAJOR >= 13
    PassBuilderOptions opts = LLVMCreatePassBuilderOptions();
    Error err = LLVMRunPasses(ura.module, passes, NULL, opts);
    if (err) {
...
#else
    // passes is "default<O?>": the level is the character after the 'O'
    char *o = strchr(passes, 'O');
    char level = o ? o[1] : '2';
    unsigned opt = level >= '0' && level <= '3' ? (unsigned)(level - '0') : 2;
    unsigned size = level == 's' ? 1 : level == 'z' ? 2 : 0;
    // the inliner thresholds clang uses for -O2, -O3, -Os and -Oz
    unsigned inline_at = size == 2 ? 25 : size == 1 ? 75 : opt == 3 ? 250 : 225;

    LLVMPassManagerBuilderRef builder = LLVMPassManagerBuilderCreate();
    LLVMPassManagerBuilderSetOptLevel(builder, opt);
    LLVMPassManagerBuilderSetSizeLevel(builder, size);
    if (opt > 1)
        LLVMPassManagerBuilderUseInlinerWithThreshold(builder, inline_at);
    LLVMPassManagerRef pm = LLVMCreatePassManager();
    LLVMPassManagerBuilderPopulateModulePassManager(builder, pm);
    LLVMRunPassManager(pm, ura.module);
...
void guard(Token *op, Value is_bad, char *what) {
    ...
    Value write_args[3] = { LLVMConstInt(ura.i32, 2, 0), msg,
                            LLVMConstInt(ura.i64, strlen(text), 0) };
    if (write_fn)
...
void guard_nonzero(Token *op, Value divisor) {
    ...
    Value iszero =
        fp ? LLVMBuildFCmp(ura.builder, LLVMRealOEQ, divisor, LLVMConstReal(ty, 0.0), "iszero")
           : LLVMBuildICmp(ura.builder, LLVMIntEQ, divisor, LLVMConstInt(ty, 0, 0), "iszero");
    guard(op, iszero,
          includes(op->type, MOD, MOD_ASSIGN, 0) ? "Modulo by zero" : "Division by zero");
}

void guard_bound(Token *op, Value ptr) {
    Value null = LLVMConstNull(LLVMTypeOf(ptr));
    Value isnull = LLVMBuildICmp(ura.builder, LLVMIntEQ, ptr, null, "unbound");
    char *msg = format("reference '%s' used before it was bound - assign '%s = ref <target>' first",
                       op->name, op->name);
    guard(op, isnull, msg);
    free(msg);
}
...
void emit_signature(Node *fn) {
    ...
    if (is_main(token)) {
        TypeRef argv = LLVMPointerType(LLVMPointerType(ura.i8, 0), 0);
...
Value field_ptr(Node *node) {
    ...
    Value idx[2] = { LLVMConstInt(ura.i32, 0, 0), LLVMConstInt(ura.i32, token->Struct.index, 0) };
    return LLVMBuildGEP2(ura.builder, sty, base, idx, 2, token->name);
}
...
Value access_ptr(Node *node) {
    ...
    if (node->token->is_nullable) {
        ...
        Value low =
            LLVMBuildICmp(ura.builder, LLVMIntSLT, idx64, LLVMConstInt(ura.i64, 0, 0), "oob.low");
        Value high = LLVMBuildICmp(ura.builder, LLVMIntSGE, idx64, len, "oob.high");
        guard(node->token, LLVMBuildOr(ura.builder, low, high, "oob"), "array index out of bounds");
    }
    Token *arr = node->left->token;
...
Value string_slice(Token *token, char *text) {
    Value str = LLVMBuildGlobalStringPtr(ura.builder, text, "str");
...
Value array_calloc(TypeRef elem, Value count, Value esz) {
    ...
    if (!fn)
        return LLVMConstNull(LLVMPointerType(elem, 0));
    Value mem = LLVMBuildCall2(ura.builder, cty, fn, (Value[]){ count, esz }, 2, "heap");
...
Value build_array(Token *arr, Value *dims, int depth, bool heap) {
    ...
    Value esz =
        LLVMConstInt(ura.i64, LLVMABISizeOfType(LLVMGetModuleDataLayout(ura.module), elem), 0);
    if (depth == 1) {
        if (heap)
            return make_slice(arr, 1, array_calloc(elem, n, esz), n);
    ...
    TypeRef inner = elem;
    Value data =
        heap ? array_calloc(inner, n, esz) : LLVMBuildArrayAlloca(ura.builder, inner, n, "arr");
    Value fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
    ...
    LLVMBuildStore(ura.builder, LLVMBuildAdd(ura.builder, i, LLVMConstInt(ura.i64, 1, 0), "next"),
                   slot);
    LLVMBuildBr(ura.builder, cond);
...
void free_array(Token *arr, Value slice, int depth) {
    ...
    if (depth > 1) {
        ...
        LLVMPositionBuilderAtEnd(ura.builder, cond);
        Value i = LLVMBuildLoad2(ura.builder, ura.i64, slot, "i");
        LLVMBuildCondBr(ura.builder, LLVMBuildICmp(ura.builder, LLVMIntSLT, i, len, "more"), body,
                        end);
        LLVMPositionBuilderAtEnd(ura.builder, body);
        Value gep = LLVMBuildGEP2(ura.builder, inner, data, &i, 1, "free.slot");
        free_array(arr, LLVMBuildLoad2(ura.builder, inner, gep, "inner"), depth - 1);
        i = LLVMBuildLoad2(ura.builder, ura.i64, slot, "i");
        LLVMBuildStore(ura.builder,
                       LLVMBuildAdd(ura.builder, i, LLVMConstInt(ura.i64, 1, 0), "next"), slot);
        LLVMBuildBr(ura.builder, cond);
        LLVMPositionBuilderAtEnd(ura.builder, end);
    }
    ...
    if (ffn)
        LLVMBuildCall2(ura.builder, fty, ffn, (Value[]){ ptr }, 1, "");
}
...
bool needs_drop(Node *def) {
    ...
    for (int i = 0; i < def->children_count; i++) {
        ...
        if (field->ret_type != STRUCT_CALL)
            continue;
        if (needs_drop(field->Struct.ptr))
            return true;
    }
    return false;
}
...
void emit_drop_value(Value ptr, Node *def) {
    ...
    if (def->token->has_drop) {
        ...
        emit_signature(fn);
        Token *impl = fn->token;
        LLVMBuildCall2(ura.builder, impl->llvm.func_type, impl->llvm.elem, &ptr, 1, "");
    }
    ...
    for (int i = def->children_count - 1; i >= 0; i--) {
        ...
        if (!needs_drop(sub))
            continue;
        Value idx[2] = { LLVMConstInt(ura.i32, 0, 0),
                         LLVMConstInt(ura.i32, field->Struct.index, 0) };
        Value slot = LLVMBuildGEP2(ura.builder, sty, ptr, idx, 2, field->name);
        emit_drop_value(slot, sub);
    }
}

void emit_drops(Node *scope, Token *keep) {
    if (!scope || LLVMGetBasicBlockTerminator(LLVMGetInsertBlock(ura.builder)))
        return;
    for (int i = scope->variables_count - 1; i >= 0; i--) {
        Token *var = scope->variables[i];
        bool skip = var == keep || var->is_ref || var->is_param || !var->llvm.elem ||
                    var->ret_type != STRUCT_CALL;
        Node *def = NULL;
        if (!skip && needs_drop(var->Struct.ptr))
            def = var->Struct.ptr;
        if (def)
            emit_drop_value(var->llvm.elem, def);
    }
}
...
void emit_unwind(Node *stop, Token *keep) {
    for (int i = ura.scopes_count; i >= 1; i--) {
        ...
        bool at_fdec = scope->token->type == FN_DEC;
        if (stop ? scope == stop : at_fdec && !is_main(scope->token))
            return;
    }
}
...
bool is_main(Token *token) {
    return token->type == FN_DEC && strcmp(token->name, "main") == 0;
}
...
Type category(Node *n) {
    Token *t = n->token;
    if (t->ret_type == ARRAY_TYPE)
        return CAT_SLICE;
    if (t->type == REF || t->type == NULL_LIT)
        return CAT_REF;
    if (t->type == NEW)
        return CAT_REF;
    if (t->type == FN_CALL && t->is_ref)
        return CAT_REF;
    return CAT_VALUE;
}
...
Value emit_place(Node *n) {
    ...
    if (t->is_dec) {
        ...
        return t->llvm.elem = LLVMBuildAlloca(ura.builder, ty, t->name);
    }
    Token *decl = t->Decl.ptr;
    if (decl->is_ref) {
        TypeRef pty = LLVMPointerType(llvm_type_of(decl), 0);
        Value ptr = LLVMBuildLoad2(ura.builder, pty, decl->llvm.elem, "ref");
        if (t->is_nullable)
            guard_bound(t, ptr);
        return ptr;
    }
...
Value emit_ref(Node *n) {
    Token *t = n->token;
    if (t->type == REF)
        return emit_ref(n->left);
    if (t->type == NULL_LIT)
        return LLVMConstNull(LLVMPointerType(llvm_type_of(t), 0));
    if (t->type == FN_CALL) {
        code_gen(n);
        return t->llvm.elem;
    }
    ...
    if (t->type == DOT && t->is_ref) {
        TypeRef pty = LLVMPointerType(llvm_type_of(t), 0);
        return LLVMBuildLoad2(ura.builder, pty, field_ptr(n), "ref");
    }
    bool ref_var = t->type == ID && t->Decl.ptr && t->Decl.ptr->is_ref;
    if (ref_var) {
        TypeRef pty = LLVMPointerType(llvm_type_of(t->Decl.ptr), 0);
        return LLVMBuildLoad2(ura.builder, pty, t->Decl.ptr->llvm.elem, "ref");
    }
    return emit_place(n);
}
...
void code_gen_operator(Node *node) {
    ...
    if (spelled) {
        ...
        arg = by_ref ? addr : LLVMBuildLoad2(ura.builder, llvm_type_of(param), addr, "deref");
        ...
        if (by_ref) {
            Value slot = LLVMBuildAlloca(ura.builder, LLVMTypeOf(arg), "op.tmp");
            LLVMBuildStore(ura.builder, arg, slot);
            arg = slot;
        }
    }
    ...
    token->llvm.elem =
        LLVMBuildCall2(ura.builder, fn->token->llvm.func_type, fn->token->llvm.elem, args, 2, name);
}

Value opt_ptr(Token *token, Value v) {
    return token->ret_type == ARRAY_TYPE ? LLVMBuildExtractValue(ura.builder, v, 0, "opt.ptr") : v;
}
...
Value emit_printf_fd(int fd, char *fmt, Value *args, int n) {
    ...
    if (err)
        call[0] = LLVMConstInt(ura.i32, fd, 0);
    call[lead - 1] = LLVMBuildGlobalStringPtr(ura.builder, fmt, "fmt");
    if (n)
        memcpy(call + lead, args, n * sizeof(Value));
    Value res = LLVMBuildCall2(ura.builder, ty, fn, call, n + lead, "");
    free(call);
    return res;
}
...
TypeRef out_frame_type() {
    ...
    TypeRef body[3] = { LLVMPointerType(ura.i8, 0), ura.i32, LLVMPointerType(t, 0) };
...
void emit_out_call(Node *def, Value ptr, Value frame, int fd) {
    ...
    if (printer) {
        ...
        Value self = ptr;
        Value slice =
            LLVMBuildCall2(ura.builder, fn->llvm.func_type, fn->llvm.elem, &self, 1, "out");
        Value len = LLVMBuildExtractValue(ura.builder, slice, 1, "out.len");
        Value data = LLVMBuildExtractValue(ura.builder, slice, 0, "out.data");
    ...
    if (!*memo) {
        ...
        TypeRef fty = LLVMFunctionType(
            ura.vd, (TypeRef[]){ LLVMPointerType(sty, 0), LLVMPointerType(frt, 0) }, 2, 0);
        ...
        *memo = fn;
        ...
        Block entry = LLVMAppendBasicBlockInContext(ura.context, fn, "entry");
        LLVMPositionBuilderAtEnd(ura.builder, entry);
        Value self = LLVMGetParam(fn, 0);
        Value parent = LLVMGetParam(fn, 1);
        Value me = LLVMBuildBitCast(ura.builder, self, LLVMPointerType(ura.i8, 0), "me");

        Value walk = LLVMBuildAlloca(ura.builder, LLVMPointerType(frt, 0), "walk");
        LLVMBuildStore(ura.builder, parent, walk);
        Block sc = LLVMAppendBasicBlockInContext(ura.context, fn, "seen.cond");
        Block sb = LLVMAppendBasicBlockInContext(ura.context, fn, "seen.body");
        Block hit = LLVMAppendBasicBlockInContext(ura.context, fn, "seen.hit");
        Block miss = LLVMAppendBasicBlockInContext(ura.context, fn, "seen.next");
        Block fresh = LLVMAppendBasicBlockInContext(ura.context, fn, "seen.fresh");
        LLVMBuildBr(ura.builder, sc);
        LLVMPositionBuilderAtEnd(ura.builder, sc);
        Value node = LLVMBuildLoad2(ura.builder, LLVMPointerType(frt, 0), walk, "q");
        Value done = LLVMBuildICmp(ura.builder, LLVMIntEQ,
                                   LLVMBuildPtrToInt(ura.builder, node, ura.i64, "q2i"),
                                   LLVMConstInt(ura.i64, 0, 0), "atroot");
        LLVMBuildCondBr(ura.builder, done, fresh, sb);
        LLVMPositionBuilderAtEnd(ura.builder, sb);
        Value pidx[2] = { LLVMConstInt(ura.i32, 0, 0), LLVMConstInt(ura.i32, 0, 0) };
        Value tidx[2] = { LLVMConstInt(ura.i32, 0, 0), LLVMConstInt(ura.i32, 1, 0) };
        Value nidx[2] = { LLVMConstInt(ura.i32, 0, 0), LLVMConstInt(ura.i32, 2, 0) };
        static int next = 1;
        if (!token->Struct.index)
            token->Struct.index = next++;
        Value myid = LLVMConstInt(ura.i32, token->Struct.index, 0);
        Value held =
            LLVMBuildLoad2(ura.builder, LLVMPointerType(ura.i8, 0),
                           LLVMBuildGEP2(ura.builder, frt, node, pidx, 2, "q.ptr"), "held");
        Value hid = LLVMBuildLoad2(
            ura.builder, ura.i32, LLVMBuildGEP2(ura.builder, frt, node, tidx, 2, "q.ty"), "heldty");
        Value hit_ptr = LLVMBuildICmp(
            ura.builder, LLVMIntEQ, LLVMBuildPtrToInt(ura.builder, held, ura.i64, "h2i"),
            LLVMBuildPtrToInt(ura.builder, me, ura.i64, "m2i"), "sameptr");
        Value hit_ty = LLVMBuildICmp(ura.builder, LLVMIntEQ, hid, myid, "samety");
        Value same = LLVMBuildBinOp(ura.builder, LLVMAnd, hit_ptr, hit_ty, "same");
        LLVMBuildCondBr(ura.builder, same, hit, miss);
        LLVMPositionBuilderAtEnd(ura.builder, hit);
        emit_printf_fd(fd, "[Circular]", NULL, 0);
        LLVMBuildRetVoid(ura.builder);
        LLVMPositionBuilderAtEnd(ura.builder, miss);
        LLVMBuildStore(ura.builder,
                       LLVMBuildLoad2(ura.builder, LLVMPointerType(frt, 0),
                                      LLVMBuildGEP2(ura.builder, frt, node, nidx, 2, "q.prev"),
                                      "up"),
                       walk);
        LLVMBuildBr(ura.builder, sc);

        LLVMPositionBuilderAtEnd(ura.builder, fresh);
        Value frame = LLVMBuildAlloca(ura.builder, frt, "frame");
        LLVMBuildStore(ura.builder, me, LLVMBuildGEP2(ura.builder, frt, frame, pidx, 2, "f.ptr"));
        LLVMBuildStore(ura.builder, myid, LLVMBuildGEP2(ura.builder, frt, frame, tidx, 2, "f.ty"));
        LLVMBuildStore(ura.builder, parent,
                       LLVMBuildGEP2(ura.builder, frt, frame, nidx, 2, "f.prev"));
        ...
        for (int i = 0; i < def->children_count; i++) {
            ...
            Value idx[2] = { LLVMConstInt(ura.i32, 0, 0),
                             LLVMConstInt(ura.i32, field->Struct.index, 0) };
            Value slot = LLVMBuildGEP2(ura.builder, sty, self, idx, 2, field->name);
            if (field->is_ref) {
                TypeRef tty = struct_type_of(field->Struct.ptr);
                Value ptr = LLVMBuildLoad2(ura.builder, LLVMPointerType(tty, 0), slot, "ref");
                Value fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
                Block nb = LLVMAppendBasicBlockInContext(ura.context, fn, "out.null");
                Block vb = LLVMAppendBasicBlockInContext(ura.context, fn, "out.ref");
                Block cb = LLVMAppendBasicBlockInContext(ura.context, fn, "out.refend");
                Value iv = LLVMBuildPtrToInt(ura.builder, ptr, ura.i64, "p2i");
                LLVMBuildCondBr(ura.builder,
                                LLVMBuildICmp(ura.builder, LLVMIntEQ, iv,
                                              LLVMConstInt(ura.i64, 0, 0), "isnull"),
                                nb, vb);
                LLVMPositionBuilderAtEnd(ura.builder, nb);
                emit_printf_fd(fd, "null", NULL, 0);
                LLVMBuildBr(ura.builder, cb);
                LLVMPositionBuilderAtEnd(ura.builder, vb);
                emit_printf_fd(fd, "ref ", NULL, 0);
                emit_out_call(field->Struct.ptr, ptr, frame, fd);
                LLVMBuildBr(ura.builder, cb);
                LLVMPositionBuilderAtEnd(ura.builder, cb);
                ...
                char *spec = NULL;
                Value v = LLVMBuildLoad2(ura.builder, llvm_type_of(field), slot, "f");
                v = print_adapt(field->ret_type, v, &spec);
                if (!v)
                    emit_printf_fd(fd, "?", NULL, 0);
                else
                    emit_printf_fd(fd, spec, (Value[]){ v }, 1);
            }
        }
        emit_printf_fd(fd, "}", NULL, 0);
        LLVMBuildRetVoid(ura.builder);
        if (prev)
            LLVMPositionBuilderAtEnd(ura.builder, prev);
    }
    TypeRef fty = def->token->llvm.func_type;
    LLVMBuildCall2(ura.builder, fty, *memo, (Value[]){ ptr, frame }, 2, "");
}

void emit_out_array(Token *field, Value slot, Value frame, int depth, int fd) {
    Value slice = LLVMBuildLoad2(ura.builder, array_type(field, depth), slot, "arr");
    Value data = LLVMBuildExtractValue(ura.builder, slice, 0, "arr.data");
    Value len = LLVMBuildExtractValue(ura.builder, slice, 1, "arr.len");
    ...
    Value fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
    Value slot_i = LLVMBuildAlloca(ura.builder, ura.i64, "oi");
    LLVMBuildStore(ura.builder, LLVMConstInt(ura.i64, 0, 0), slot_i);
    emit_printf_fd(fd, "[", NULL, 0);
    Block cond = LLVMAppendBasicBlockInContext(ura.context, fn, "out.arr.cond");
    Block body = LLVMAppendBasicBlockInContext(ura.context, fn, "out.arr.body");
    Block sep = LLVMAppendBasicBlockInContext(ura.context, fn, "out.arr.sep");
    Block item = LLVMAppendBasicBlockInContext(ura.context, fn, "out.arr.item");
    Block end = LLVMAppendBasicBlockInContext(ura.context, fn, "out.arr.end");
    LLVMBuildBr(ura.builder, cond);
    LLVMPositionBuilderAtEnd(ura.builder, cond);
    Value i = LLVMBuildLoad2(ura.builder, ura.i64, slot_i, "i");
    LLVMBuildCondBr(ura.builder, LLVMBuildICmp(ura.builder, LLVMIntSLT, i, len, "more"), body, end);
    LLVMPositionBuilderAtEnd(ura.builder, body);
    LLVMBuildCondBr(
        ura.builder,
        LLVMBuildICmp(ura.builder, LLVMIntSGT, i, LLVMConstInt(ura.i64, 0, 0), "notfirst"), sep,
        item);
    LLVMPositionBuilderAtEnd(ura.builder, sep);
    emit_printf_fd(fd, ", ", NULL, 0);
    LLVMBuildBr(ura.builder, item);
    LLVMPositionBuilderAtEnd(ura.builder, item);
    Value at = LLVMBuildGEP2(ura.builder, ety, data, &i, 1, "at");
    if (depth > 1)
        ...
        char *spec = NULL;
        Value v =
            print_adapt(field->Array.sub_type, LLVMBuildLoad2(ura.builder, ety, at, "e"), &spec);
        if (v)
            emit_printf_fd(fd, spec, (Value[]){ v }, 1);
        else
            emit_printf_fd(fd, "?", NULL, 0);
    }
    LLVMBuildStore(ura.builder, LLVMBuildAdd(ura.builder, i, LLVMConstInt(ura.i64, 1, 0), "n"),
                   slot_i);
    LLVMBuildBr(ura.builder, cond);
    LLVMPositionBuilderAtEnd(ura.builder, end);
    emit_printf_fd(fd, "]", NULL, 0);
...
Value struct_arg_ptr(Node *arg) {
    ...
    if (includes(token->type, ID, ACCESS, DOT, 0) && !token->is_dec) {
        ...
        if (token->type != DOT || !token->is_ref)
            return slot;
        TypeRef sty = LLVMPointerType(struct_type_of(token->Struct.ptr), 0);
        Value ptr = LLVMBuildLoad2(ura.builder, sty, slot, "ref");
        if (token->is_nullable)
            guard_bound(token, ptr);
        return ptr;
    }
    code_gen(arg);
    if (category(arg) == CAT_REF)
        return token->llvm.elem;
    Value tmp = LLVMBuildAlloca(ura.builder, struct_type_of(token->Struct.ptr), "out.tmp");
    LLVMBuildStore(ura.builder, token->llvm.elem, tmp);
...
bool program_throws(Node *n) {
    if (!n)
        return false;
    if (includes(n->token->type, THROW, TRY, 0))
        return true;
    // break/continue->left is a back-edge to the loop; recursing loops forever
    if (includes(n->token->type, BREAK, CONTINUE, 0))
        return false;
    if (program_throws(n->left) || program_throws(n->right))
        return true;
    for (int i = 0; i < n->children_count; i++)
        if (program_throws(n->children[i]))
            return true;
    return false;
}
...
void emit_uncaught() {
    ...
    Value idx[2] = { LLVMConstInt(ura.i32, 0, 0), LLVMConstInt(ura.i32, 0, 0) };
    Value msgp = LLVMBuildGEP2(ura.builder, et, ura.err_value, idx, 2, "err.msg");
    Value msg = LLVMBuildLoad2(ura.builder, mt, msgp, "msg");
    Value data = LLVMBuildExtractValue(ura.builder, msg, 0, "msg.data");
    Value len = LLVMBuildExtractValue(ura.builder, msg, 1, "msg.len");
    TypeRef wty = NULL;
    Value wfn = lib_fn("write", &wty);
    if (wfn)
        LLVMBuildCall2(ura.builder, wty, wfn, (Value[]){ LLVMConstInt(ura.i32, 2, 0), data, len },
                       3, "");
    TypeRef xty = NULL;
    Value xfn = lib_fn("exit", &xty);
    if (xfn)
        LLVMBuildCall2(ura.builder, xty, xfn, (Value[]){ LLVMConstInt(ura.i32, 1, 0) }, 1, "");
    LLVMBuildUnreachable(ura.builder);
}

Node *current_fdec() {
    for (int i = ura.scopes_count; i >= 1; i--)
        if (ura.scopes[i] && ura.scopes[i]->token->type == FN_DEC)
            return ura.scopes[i];
    return NULL;
}
...
void emit_throw_branch() {
    ...
    if (ura.try_nodes_count > 0) {
        ...
        emit_unwind(tn, NULL);
        LLVMBuildBr(ura.builder, tn->token->llvm._catch);
        return;
    }
...
void code_gen(Node *node) {
    ...
    case FN_DEC: {
        ...
        LLVMPositionBuilderAtEnd(
            ura.builder, LLVMAppendBasicBlockInContext(ura.context, token->llvm.elem, "entry"));
        if (ura.enable_san) {
        ...
        if (ura.debug_builder) {
            ...
            MetadataRef di_func = LLVMDIBuilderCreateFunction(
                ura.debug_builder, ura.debug_compile_unit, token->name, len, token->name, len,
                ura.debug_file, token->line, di_type, 0, 1, token->line, LLVMDIFlagZero, 0);
            LLVMSetSubprogram(token->llvm.elem, di_func);
            ura.debug_scope = di_func;
            LLVMSetCurrentDebugLocation2(
                ura.builder,
                LLVMDIBuilderCreateDebugLocation(ura.context, token->line, 0, di_func, NULL));
        }
        enter_scope(node);
        for (int i = 0; i < token->Fn.params_count; i++) {
            Token *param = token->Fn.params[i];
        ...
        if (is_main(token)) {
            for (int i = 0; i < ura.head->children_count; i++) {
                ...
                if (!os || !os->name || strcmp(os->name, "os") != 0)
                    continue;
                if (os->ret_type != STRUCT_CALL || !os->Struct.ptr)
                    break;
                if (!os->used)
                    break;
                Value fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
                {
                    ...
                    LLVMBuildStore(ura.builder, argc,
                                   LLVMBuildStructGEP2(ura.builder, sty, base, 0, "os.argc"));
                    ...
                    Value raw = LLVMBuildCall2(ura.builder, cty, cfn,
                                               (Value[]){ n64, LLVMConstInt(ura.i64, 16, 0) }, 2,
                                               "argvbuf");
                    Value buf =
                        LLVMBuildBitCast(ura.builder, raw, LLVMPointerType(strt, 0), "argv.buf");

                    TypeRef lty = NULL;
                    Value lfn = lib_fn("strlen", &lty);
                    Block cond = LLVMAppendBasicBlockInContext(ura.context, fn, "os.cond");
                    Block body = LLVMAppendBasicBlockInContext(ura.context, fn, "os.body");
                    Block end = LLVMAppendBasicBlockInContext(ura.context, fn, "os.end");
                    Value slot = LLVMBuildAlloca(ura.builder, ura.i64, "oi");
                    LLVMBuildStore(ura.builder, LLVMConstInt(ura.i64, 0, 0), slot);
                    LLVMBuildBr(ura.builder, cond);
                    LLVMPositionBuilderAtEnd(ura.builder, cond);
                    Value i = LLVMBuildLoad2(ura.builder, ura.i64, slot, "i");
                    LLVMBuildCondBr(ura.builder,
                                    LLVMBuildICmp(ura.builder, LLVMIntSLT, i, n64, "more"), body,
                                    end);
                    LLVMPositionBuilderAtEnd(ura.builder, body);
                    TypeRef i8p = LLVMPointerType(ura.i8, 0);
                    Value src =
                        LLVMBuildLoad2(ura.builder, i8p,
                                       LLVMBuildGEP2(ura.builder, i8p, argv, &i, 1, "ap"), "arg");
                    Value len = LLVMBuildCall2(ura.builder, lty, lfn, (Value[]){ src }, 1, "alen");
                    Value sl =
                        LLVMBuildInsertValue(ura.builder, LLVMGetUndef(strt), src, 0, "a.ptr");
                    sl = LLVMBuildInsertValue(ura.builder, sl, len, 1, "a.len");
                    LLVMBuildStore(ura.builder, sl,
                                   LLVMBuildGEP2(ura.builder, strt, buf, &i, 1, "slot"));
                    LLVMBuildStore(
                        ura.builder,
                        LLVMBuildBinOp(ura.builder, LLVMAdd, i, LLVMConstInt(ura.i64, 1, 0), "inc"),
                        slot);
                    LLVMBuildBr(ura.builder, cond);
                    LLVMPositionBuilderAtEnd(ura.builder, end);
                    Value agg =
                        LLVMBuildInsertValue(ura.builder, LLVMGetUndef(outer), buf, 0, "argv.ptr");
                    agg = LLVMBuildInsertValue(ura.builder, agg, n64, 1, "argv.len");
                    LLVMBuildStore(ura.builder, agg,
                                   LLVMBuildStructGEP2(ura.builder, sty, base, 1, "os.argv"));
                }
                break;
            }
            for (int i = 0; i < ura.head->children_count; i++) {
                Node *child = ura.head->children[i];
                if (child->token->type != ASSIGN || !global_decl(child))
                    continue;
                code_gen(child);
                drop_temps();
            }
        ...
        for (int i = 0; i < node->children_count; i++) {
            ...
            if (ura.debug_builder && ura.debug_scope)
                LLVMSetCurrentDebugLocation2(
                    ura.builder, LLVMDIBuilderCreateDebugLocation(ura.context, st->line, 0,
                                                                  ura.debug_scope, NULL));
            code_gen(node->children[i]);
            drop_temps();
        ...
        scope_out();
        if (is_main(token))
            emit_drops(ura.head, NULL);
        if (!LLVMGetBasicBlockTerminator(LLVMGetInsertBlock(ura.builder))) {
            if (token->ret_type == VOID)
                LLVMBuildRetVoid(ura.builder);
            else
                LLVMBuildRet(ura.builder, default_value(token));
        }
        if (token->llvm.prev_block)
            LLVMPositionBuilderAtEnd(ura.builder, token->llvm.prev_block);
        if (ura.debug_builder) {
    ...
    case FN_CALL: {
        ...
        if (n > 0) {
            ...
            if (self)
                args[0] = struct_arg_ptr(node->left);
            for (int i = 0; i < node->children_count; i++) {
                ...
                bool keep = arg->ret_type != ARRAY_TYPE || want == ARRAY_TYPE || !is_pointer(want);
                args[i + self] = keep ? dv : LLVMBuildExtractValue(ura.builder, dv, 0, "arr.data");
        ...
        if (indirect) {
            ...
            Value isnull = LLVMBuildICmp(ura.builder, LLVMIntEQ, fn_ptr, null, "isnull");
            guard(token, isnull, "Call to a null function value");
            char *name = fn->Fn.ret->ret_type == VOID ? "" : "call";
            token->llvm.elem =
                LLVMBuildCall2(ura.builder, LLVMGetElementType(ptr_type), fn_ptr, args, n, name);
        } else {
            char *name = fn->ret_type == VOID ? "" : "call";
            token->llvm.elem =
                LLVMBuildCall2(ura.builder, fn->llvm.func_type, fn->llvm.elem, args, n, name);
        }
        free(args);
        ...
        if (ura.uses_exceptions) {
            ensure_err_globals();
            Value fnv = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
            Block unwind = LLVMAppendBasicBlockInContext(ura.context, fnv, "throw.unwind");
            Block cont = LLVMAppendBasicBlockInContext(ura.context, fnv, "throw.cont");
            Value flag = LLVMBuildLoad2(ura.builder, ura.i1, ura.err_flag, "eflag");
            LLVMBuildCondBr(ura.builder, flag, unwind, cont);
            LLVMPositionBuilderAtEnd(ura.builder, unwind);
            emit_throw_branch();
            LLVMPositionBuilderAtEnd(ura.builder, cont);
        }
        break;
    ...
    case OUTPUT: {
        ...
        if (fd == 2)
            emit_printf_fd(fd, "\033[0;31m", NULL, 0);
        for (int i = 0; i < node->children_count; i++) {
            ...
            if (token->ret_type == STRUCT_CALL) {
                if (fc) {
                    fmt[fc] = 0;
                    last = emit_printf_fd(fd, fmt, args, nargs);
                }
                fc = 0;
                nargs = 0;
                emit_out_call(token->Struct.ptr, struct_arg_ptr(arg),
                              LLVMConstNull(LLVMPointerType(out_frame_type(), 0)), fd);
                continue;
            }
            code_gen(arg);
            if (is_string(token)) {
                Value len = LLVMBuildExtractValue(ura.builder, token->llvm.elem, 1, "str.len");
                args[nargs++] = LLVMBuildTrunc(ura.builder, len, ura.i32, "len32");
                args[nargs++] = LLVMBuildExtractValue(ura.builder, token->llvm.elem, 0, "str.data");
                for (char *s = "%.*s"; *s; s++)
                    fmt[fc++] = *s;
                continue;
            }
            if (token->ret_type == ARRAY_TYPE) {
                ...
                LLVMBuildStore(ura.builder, token->llvm.elem, slot);
                emit_out_array(token, slot, LLVMConstNull(LLVMPointerType(out_frame_type(), 0)),
                               depth, fd);
                continue;
            }
            char *spec = NULL;
        ...
        if (fd == 2)
            last = emit_printf_fd(fd, "\033[0m", NULL, 0);
    ...
    case ARRAY: {
        ...
        for (int i = 0; i < depth; i++) {
            code_gen(node->children[i]);
            dims[i] = LLVMBuildIntCast2(ura.builder, node->children[i]->token->llvm.elem, ura.i64,
                                        1, "n");
        }
    ...
    case NEW: {
        ...
        Value esz =
            LLVMConstInt(ura.i64, LLVMABISizeOfType(LLVMGetModuleDataLayout(ura.module), sty), 0);
        token->llvm.elem = array_calloc(sty, LLVMConstInt(ura.i64, 1, 0), esz);
        break;
    }
    case ACCESS: {
        if (node->right->token->type == RANGE) {
            ...
            Value start =
                LLVMBuildIntCast2(ura.builder, range->left->token->llvm.elem, ura.i64, 1, "start");
            Value end =
                LLVMBuildIntCast2(ura.builder, range->right->token->llvm.elem, ura.i64, 1, "end");
            if (node->token->is_nullable) {
                Value len = LLVMBuildExtractValue(ura.builder, slice, 1, "arr.len");
                Value lo = LLVMBuildICmp(ura.builder, LLVMIntSLT, start,
                                         LLVMConstInt(ura.i64, 0, 0), "s.lo");
                Value hi = LLVMBuildICmp(ura.builder, LLVMIntSGT, end, len, "e.hi");
                Value ord = LLVMBuildICmp(ura.builder, LLVMIntSGT, start, end, "s.gt");
                Value bad =
                    LLVMBuildOr(ura.builder, LLVMBuildOr(ura.builder, lo, hi, "b"), ord, "bad");
    ...
    case DOT: {
        ...
        if (node->left->token->ret_type == STRUCT_CALL) {
            ...
            token->llvm.elem = LLVMBuildLoad2(ura.builder, llvm_type_of(token), ptr, token->name);
            return;
        }
        code_gen(node->left);
        token->llvm.elem =
            LLVMBuildExtractValue(ura.builder, node->left->token->llvm.elem, 1, "len");
        break;
    }
    case TYPEOF: {
        Token *arg = node->left->token;
        char *name = arg->ret_type == STRUCT_CALL ? struct_name_of(arg) : type_name(arg->ret_type);
    ...
    case SIZEOF: {
        ...
        node->token->llvm.elem = LLVMConstInt(ura.i64, sz, 0);
        break;
    }
    case CLEAN: {
        ...
        if (tgt->ret_type == STRUCT_CALL && tgt->is_ref) {
            ...
            Value fnv = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
            Block live = LLVMAppendBasicBlockInContext(ura.context, fnv, "clean.live");
            Block done = LLVMAppendBasicBlockInContext(ura.context, fnv, "clean.done");
            Value nil = LLVMBuildICmp(ura.builder, LLVMIntEQ, ptr, LLVMConstNull(pty), "isnull");
            LLVMBuildCondBr(ura.builder, nil, done, live);
            LLVMPositionBuilderAtEnd(ura.builder, live);
            emit_drop_value(ptr, tgt->Struct.ptr);
            TypeRef fty = NULL;
            Value ffn = lib_fn("free", &fty);
            Value raw = LLVMBuildBitCast(ura.builder, ptr, LLVMPointerType(ura.i8, 0), "free.ptr");
            if (ffn)
                LLVMBuildCall2(ura.builder, fty, ffn, (Value[]){ raw }, 1, "");
            LLVMBuildBr(ura.builder, done);
            LLVMPositionBuilderAtEnd(ura.builder, done);
            LLVMBuildStore(ura.builder, LLVMConstNull(pty), slot);
            break;
        }
    ...
    case FOR: {
        if (node->right->token->type != RANGE) {
            ...
            LLVMBuildCondBr(ura.builder, LLVMBuildICmp(ura.builder, LLVMIntSLT, i, len, "more"),
                            body, end);
            LLVMPositionBuilderAtEnd(ura.builder, body);
            ...
            LLVMBuildStore(ura.builder,
                           LLVMBuildAdd(ura.builder, iv, LLVMConstInt(ura.i64, 1, 0), "next"), idx);
            LLVMBuildBr(ura.builder, cond);
            LLVMPositionBuilderAtEnd(ura.builder, end);
            return;
        }
        ...
        Value mag = LLVMConstInt(ura.i32, 1, 0);
        ...
        Value more =
            LLVMBuildSelect(ura.builder, asc, LLVMBuildICmp(ura.builder, LLVMIntSLT, i, b, "lt"),
                            LLVMBuildICmp(ura.builder, LLVMIntSGT, i, b, "gt"), "more");
        LLVMBuildCondBr(ura.builder, more, body, end);
        LLVMPositionBuilderAtEnd(ura.builder, body);
        enter_scope(node);
        code_gen_body(node);
        scope_out();
    ...
    case MATCH: {
        ...
        for (int i = 0; i < node->children_count; i++) {
            ...
            Block next = i + 1 < node->children_count
                             ? LLVMAppendBasicBlockInContext(ura.context, fn, "case.next")
                             : end;
            Value cond = NULL;
            for (int j = 0; j < branch->left->children_count; j++) {
    ...
    case ASSIGN: {
        ...
        if (token->kind == REF_REBIND) {
            ...
            if (left->token->is_dec) {
                ...
                LLVMBuildStore(ura.builder, token->llvm.elem, dest);
                return;
            }
            ...
            LLVMBuildStore(ura.builder, ptr, slot);
            token->llvm.elem = ptr;
            return;
        }
    ...
    case REF:
        token->llvm.elem = emit_ref(node->left);
        break;
    case THROW: {
        ensure_err_globals();
        code_gen(node->left);
        LLVMBuildStore(ura.builder, node->left->token->llvm.elem, ura.err_value);
        LLVMBuildStore(ura.builder, LLVMConstInt(ura.i1, 1, 0), ura.err_flag);
        drop_temps();
        emit_throw_branch();
        break;
    }
    case TRY: {
        ensure_err_globals();
        Value fn = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ura.builder));
        Block cblk = LLVMAppendBasicBlockInContext(ura.context, fn, "catch");
        Block end = LLVMAppendBasicBlockInContext(ura.context, fn, "try.end");
        ...
        if (!LLVMGetBasicBlockTerminator(LLVMGetInsertBlock(ura.builder)))
            LLVMBuildBr(ura.builder, end);
        LLVMPositionBuilderAtEnd(ura.builder, cblk);
        Node *cnode = node->right;
        enter_scope(cnode);
        Value slot = emit_place(cnode->left);
        TypeRef et = struct_type_of(ura.error_def);
        LLVMBuildStore(ura.builder, LLVMBuildLoad2(ura.builder, et, ura.err_value, "ev"), slot);
        LLVMBuildStore(ura.builder, LLVMConstInt(ura.i1, 0, 0), ura.err_flag);
        code_gen_body(cnode);
        scope_out();
    ...
    case BREAK: {
        ...
        LLVMBuildBr(ura.builder, node->left->token->llvm.end);
        break;
    }
    case CONTINUE: {
        drop_temps();
        emit_unwind(node->left, NULL);
        LLVMBuildBr(ura.builder, node->left->token->llvm.start);
        break;
    }
    case I32:
    case BOOL:
    ...
    case NULL_LIT: {
        ...
        case I32:
            ...
            break;
        case BOOL:
            token->llvm.elem = LLVMConstInt(to_llvm_type(token->ret_type), token->Bool.value, 0);
            break;
        case CHARS:
        ...
        case NULL_LIT:
            ...
            break;
        case CHAR:
            token->llvm.elem = LLVMConstInt(to_llvm_type(token->ret_type), token->Char.value, 0);
            break;
        case F32:
            token->llvm.elem = LLVMConstReal(to_llvm_type(token->ret_type), token->Float.value);
            break;
        default:
            break;
        }
    ...
    case AS: {
        ...
        if (src == dst) {
            token->llvm.elem = value;
            break;
        }
        TypeRef to = to_llvm_type(dst);
        if (is_float(src) && is_float(dst))
            token->llvm.elem = dst == F64 ? LLVMBuildFPExt(ura.builder, value, to, "cast")
                                          : LLVMBuildFPTrunc(ura.builder, value, to, "cast");
        else if (is_float(src))
            token->llvm.elem = is_unsigned(dst) ? LLVMBuildFPToUI(ura.builder, value, to, "cast")
                                                : LLVMBuildFPToSI(ura.builder, value, to, "cast");
        else if (is_float(dst))
            token->llvm.elem = is_unsigned(src) ? LLVMBuildUIToFP(ura.builder, value, to, "cast")
                                                : LLVMBuildSIToFP(ura.builder, value, to, "cast");
        else
            token->llvm.elem = LLVMBuildIntCast2(ura.builder, value, to, !is_unsigned(src), "cast");
        break;
    }
    case ADD_ASSIGN:
    case SUB_ASSIGN:
    ...
    case RSHIFT_ASSIGN: {
        ...
        Value current = LLVMBuildLoad2(ura.builder, type, dest, "cur");
        Value res = right;
        Type lt = node->left->token->ret_type;
        bool fp = is_float(lt);
        ...
        if (divides)
            guard_nonzero(node->token, right);
#define ARITH(fop, sop, uop, fname, iname)                             \
    res = fp ? LLVMBuildBinOp(ura.builder, fop, current, right, fname) \
             : LLVMBuildBinOp(ura.builder, un ? uop : sop, current, right, iname)
        switch (op) {
        case ADD_ASSIGN:
            ARITH(LLVMFAdd, LLVMAdd, LLVMAdd, "fadd", "add");
            break;
        case SUB_ASSIGN:
            ARITH(LLVMFSub, LLVMSub, LLVMSub, "fsub", "sub");
            break;
        case MUL_ASSIGN:
            ARITH(LLVMFMul, LLVMMul, LLVMMul, "fmul", "mul");
            break;
        case DIV_ASSIGN:
            ARITH(LLVMFDiv, LLVMSDiv, LLVMUDiv, "fdiv", "div");
            break;
        case MOD_ASSIGN:
            ARITH(LLVMFRem, LLVMSRem, LLVMURem, "frem", "mod");
            break;

        case BAND_ASSIGN:
            res = LLVMBuildBinOp(ura.builder, LLVMAnd, current, right, "band");
            break;
        case BOR_ASSIGN:
            res = LLVMBuildBinOp(ura.builder, LLVMOr, current, right, "bor");
            break;
        case BXOR_ASSIGN:
            res = LLVMBuildBinOp(ura.builder, LLVMXor, current, right, "bxor");
            break;
        case LSHIFT_ASSIGN:
            res = LLVMBuildBinOp(ura.builder, LLVMShl, current, right, "shl");
            break;
        case RSHIFT_ASSIGN:
            res = LLVMBuildBinOp(ura.builder, un ? LLVMLShr : LLVMAShr, current, right, "shr");
            break;
        default:
            break;
        }
#undef ARITH
    ...
    case RSHIFT: {
        ...
        if (token->kind == CMP_REF) {
            ...
            token->llvm.elem = LLVMBuildICmp(ura.builder, p, l, r, "refcmp");
            return;
        }
        code_gen(node->left);
        code_gen(node->right);
        ...
        if (token->kind == CMP_SLICE) {
            ...
            token->llvm.elem = LLVMBuildICmp(ura.builder, p, l, r, "nullcmp");
            return;
        }
        Type lt = node->left->token->ret_type;
        bool fp = is_float(lt);
        bool un = is_unsigned(lt);
        if (includes(token->type, DIV, MOD, 0))
            guard_nonzero(token, right);
#define ARITH(fop, sop, uop, fname, iname)                          \
    res = fp ? LLVMBuildBinOp(ura.builder, fop, left, right, fname) \
             : LLVMBuildBinOp(ura.builder, un ? uop : sop, left, right, iname)
#define CMP(fpred, spred, upred, fname, iname)                       \
    res = fp ? LLVMBuildFCmp(ura.builder, fpred, left, right, fname) \
             : LLVMBuildICmp(ura.builder, un ? upred : spred, left, right, iname)
        switch (token->type) {
        case ADD:
            ARITH(LLVMFAdd, LLVMAdd, LLVMAdd, "fadd", "add");
            break;
        ...
        case EQUAL:
            CMP(LLVMRealOEQ, LLVMIntEQ, LLVMIntEQ, "feq", "eq");
            break;
        case NOT_EQUAL:
            CMP(LLVMRealUNE, LLVMIntNE, LLVMIntNE, "fne", "ne");
            break;
        case LESS:
            CMP(LLVMRealOLT, LLVMIntSLT, LLVMIntULT, "flt", "lt");
            break;
        case GREAT:
            CMP(LLVMRealOGT, LLVMIntSGT, LLVMIntUGT, "fgt", "gt");
            break;
        case LESS_EQUAL:
            CMP(LLVMRealOLE, LLVMIntSLE, LLVMIntULE, "fle", "le");
            break;
        case GREAT_EQUAL:
            CMP(LLVMRealOGE, LLVMIntSGE, LLVMIntUGE, "fge", "ge");
            break;

        case AND:
            res = LLVMBuildBinOp(ura.builder, LLVMAnd, left, right, "and");
            break;
        case OR:
            res = LLVMBuildBinOp(ura.builder, LLVMOr, left, right, "or");
            break;
        case BAND:
            res = LLVMBuildBinOp(ura.builder, LLVMAnd, left, right, "band");
            break;
        case BOR:
            res = LLVMBuildBinOp(ura.builder, LLVMOr, left, right, "bor");
            break;
        case BXOR:
            res = LLVMBuildBinOp(ura.builder, LLVMXor, left, right, "bxor");
            break;
        case LSHIFT:
            res = LLVMBuildBinOp(ura.builder, LLVMShl, left, right, "shl");
            break;
        case RSHIFT:
            res = LLVMBuildBinOp(ura.builder, un ? LLVMLShr : LLVMAShr, left, right, "shr");
            break;
        default:
            break;
        }
...
void parse_arguments(int argc, char **argv) {
    ...
    if (!is_dir(ura.lib))
        ura.lib = find_ura_lib();
    ura.output = "exe.out";

    for (int i = 1; i < argc && !ura.error_count; i++) {
        char *arg = argv[i];
#define MATCH(name, field, val)   \
...
#undef MATCH
        if (strcmp(arg, "-o") == 0) {
            ...
            if (!is_ura)
                parse_error(NULL, "Invalid file '%s'", arg);
            else
                new_source(arg);
        }
    }
...
void load_common() {
    ...
    new_source(path);
    free(path);
    if (ura.error_count || ura.sources_count == before)
        return;
    ura.calling_use++;
    tokenize(0);
    ura.calling_use--;
...
void tokenize(int default_indent) {
    if (ura.error_count || !ura.sources)
        return;
    ...
    while (content && content[i] && !ura.error_count) {
        s = i;
        char c = content[i];
        if (lex_spaces(content, &i, &line, &indent, default_indent))
            continue;
        if (lex_multi_comment(content, &i, &line))
            continue;
        if (lex_comment(content, &i))
            continue;
        if (lex_chars(content, &i, line, indent))
            continue;
        if (lex_char(content, &i, line, indent))
            continue;
        if (lex_number(content, &i, line, indent))
            continue;
        if (lex_identifier(content, &i, line, indent, default_indent))
            continue;
        if (lex_symbol(content, &i, line, &indent))
            continue;
...
void generate_ast() {
    ...
    if (ura.error_count > 0 || !ura.enable_debug)
        return;
    debug(GREEN("===========================================\n"));
...
void declare_module_members(Node *m) {
    ...
    for (int i = 0; i < m->children_count; i++)
        if (m->children[i]->token->type == FN_DEC)
...
void generate_ir() {
    ...
    for (int i = 0; i < ura.head->children_count; i++)
        if (ura.head->children[i]->token->type == FN_DEC)
...
void generate_asm() {
    ...
    for (int i = 0; i < ura.head->children_count; i++) {
        Token *global = global_decl(ura.head->children[i]);
        if (global) {
            TypeRef type = llvm_type_of(global);
            Value glob = LLVMAddGlobal(ura.module, type, global->name);
            LLVMSetInitializer(glob, LLVMConstNull(type));
            LLVMSetLinkage(glob, LLVMInternalLinkage);
            global->llvm.elem = glob;
        }
    }
    for (int i = 0; i < ura.head->children_count; i++)
...
void compile_executable() {
    ...
    fprintf(stderr, CYAN("%-9s") " \033[2m[%s%s]\033[0m in %.2fs\n", "Finished", opt, tag,
            clock_now() - ura.time_start);
}

void run_executable() {
    if (!ura.enable_exec || ura.error_count || !ura.head)
        return;
    ...
    if (WIFSIGNALED(status))
        fprintf(stderr, RED("%-9s") " %s (signal %d) in %.2fs\n", "\nCrashed",
                signal_name(WTERMSIG(status)), WTERMSIG(status), exec);
    else
