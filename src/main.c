...
typedef struct { char *name; Type type; } Types;
...
Token *parse_token(char *filename, int line, char *input, int s, int e, Type type, int space)
{
	...
	case ID:
	{
		...
		Types keywords[] = {
			...
			{"ref",REF},{"struct",STRUCT_DEF},{"enum",ENUM_DEF},{"proto",PROTO},{"as",AS},{0,0}
		};
		...
		Types keywords2[] = {
			...
			{"stack",STACK},{"heap",HEAP},{"array",ARRAY_TYPE},{"null",NULLABLE},{0,0}
		};
...
static Node *parse_struct_def(Token *token)
{
	...
	while (within(node->token->space))
	{
		...
		if (child->token->type == FDEC)
		{
			...
			{
				if (check(child->token->retType != VOID, "struct '%s': init() must return void", node->token->name))
					return syntax_error();
				if (check(child->left->children_count != 1, "struct '%s': init() must take no parameters", node->token->name))
					return syntax_error();
				node->token->has_init = true;
			}
			else if (strcmp(child->token->name, "clean") == 0)
			{
				if (check(child->token->retType != VOID, "struct '%s': clean() must return void", node->token->name))
					return syntax_error();
				if (check(child->left->children_count != 1, "struct '%s': clean() must take no parameters", node->token->name))
					return syntax_error();
...
void gen_asm(Node *node)
{
	...
	case ASSIGN:
	{
		...
		if (left->token->is_ref)
		{
			Value ptr = LLVMBuildLoad2(builder, LLVMPointerType(type, 0), left->token->llvm.elem, "ref_ptr");
			LLVMBuildStore(builder, val, ptr);
		}
		else
			LLVMBuildStore(builder, val, left->token->llvm.elem);
	...
	case ADD_ASSIGN: case SUB_ASSIGN: case MUL_ASSIGN: case DIV_ASSIGN: case MOD_ASSIGN:
	{
		...
		if (left->token->is_ref)
		{
			...
			LLVMBuildStore(builder, result, left->token->llvm.elem);
		break;
	}
	case NOT: case BNOT:
	{
		...
		node->token->llvm.elem = LLVMBuildNot(builder, left->token->llvm.elem, to_string(node->token->type));
		break;
	}
	case ADD: case SUB: case MUL: case DIV: case EQUAL:
	case NOT_EQUAL: case LESS: case GREAT: case LESS_EQUAL:
	...
	case STACK:
	{
		...
		Value total_bytes = LLVMBuildMul(builder, total, LLVMConstInt(i32, (unsigned)elem_size, 0), "bytes");
		...
		if (count)
		{
			...
			for (int i = 0; i < fixed_params; i++)
			{
				...
				if (param_is_ref && arg_is_ref)
				{
					...
					args[i] = LLVMBuildLoad2(builder, LLVMPointerType(type, 0), argNodes[i]->token->llvm.elem, "ref_arg");
				}
	...
	case FDEC:
	{
		...
		if (strcmp(fname, "main") == 0)
		{
			...
			if (os_used)
			{
				...
				funcType = LLVMFunctionType(retType, NULL, 0, false);
		}
		else
			funcType = LLVMFunctionType(retType, paramTypes, _count, node->token->is_variadic);
	...
	case DOT:
	{
		...
		if (struct_tok->is_ref)
		{
			...
			node->token->llvm.elem = LLVMBuildGEP2(builder, 
		...
		if (false)
		{
			...
			if (left_elem_type == CHARS)
			{
				...
				if (node->left->token->name && !left->token->llvm.array_size)
				{
					...
					Value strlen_result = LLVMBuildCall2(builder, LLVMGlobalGetValueType(strlen_func), strlen_func,
	...
	case TYPEOF: case SIZEOF:
	{
		...
		if (node->token->type == TYPEOF)
		{
			node->token->llvm.elem      = LLVMBuildGlobalStringPtr(builder, node->token->Chars.value, "typeof");
			node->token->llvm.is_loaded = true;
		}
		else
		{
...
char* compile(char *path_name)
{
	...
	for(int i = 0; tokens[i]; i++) debug(GREEN("%k\n"), tokens[i]);
