...
{
   TypeRef type = get_llvm_type(token);
   if (token->is_ref) type = _pointer_type(type, 0);
...
{
   if (token->is_ref)
      _build_store(_get_default_value(token), token->llvm.elem);
}
...
void _const_value(Token *token)
{
   ...
   if (token->type == CHARS) token->llvm.elem = _const_chars(processed, name);
   else token->llvm.elem = _const_int(type, value, 0);
   free(processed);
}
...
Value getNullCheckFunc()
{
   ...
   Value is_null = _build_icmp(LLVMIntEQ,
                               _build_ptr_to_int(ptr, i64, "ptrint"),
                               _const_int(i64, 0, 0),
                               "isnull");
   ...
   if (!printf_func) printf_func = _add_function("printf", printf_type);
   ...
   if (!exit_func) exit_func = _add_function("exit", exit_type);
...
Value _build_invoke2(TypeRef ty, Value fn, Value *args, unsigned num_args,
