...
TypeRef get_base_type(Token *token)
{
   ...
   token->is_ref = false;
   TypeRef basTypeRef = get_llvm_type(token);
...
ValueRef dereference_if_ref(Token *token)
{
   ...
   TypeRef basTypeRef = get_base_type(token);
   ValueRef addr = LLVMBuildLoad2(builder,
                                  LLVMPointerType(basTypeRef, 0),
                                  token->llvm.elem,
                                  "ref_addr");

   return LLVMBuildLoad2(builder, basTypeRef, addr, "deref");
}
...
void call_function(Token *curr)
{
   ...
   if (curr->Fcall.pos)
   {
      ...
      for (int i = 0; i < curr->Fcall.pos; i++)
      {
         ...
         Token *param = curr->Fcall.ptr->Fdec.args[i];
...
{
   TypeRef basTypeRef = token->is_ref ? get_base_type(token) : get_llvm_type(token);
   return LLVMBuildLoad2(builder, basTypeRef, token->llvm.elem, token->name);
}
   ...
   if (variable->is_ref)
   {
      if (!variable->has_ref)
      {
         ...
         else
         {
            ...
            TypeRef valuTypeRef = LLVMTypeOf(rightVal);
            ValueRef allocatedSpace = LLVMBuildAlloca(builder, valuTypeRef, "ref_storage");
      ...
      {
         TypeRef basTypeRef = get_base_type(variable);
         ...
         rightVal = convert_type_if_needed(rightVal, basTypeRef);
...
{
   ValueRef param = LLVMGetParam(token->Param.ptr->llvm.elem, token->Param.index);
   LLVMSetValueName(param, token->name);

   if (token->is_ref)
   {
      TypeRef basTypeRef = get_base_type(token);
      ValueRef ref_storage = LLVMBuildAlloca(builder, LLVMPointerType(basTypeRef, 0), token->name);
      LLVMBuildStore(builder, param, ref_storage);
      token->has_ref = true;
      return ref_storage;
   }

   if (!token->Param.ptr->is_proto)
   {
...
ValueRef cast(Token *from, Token *to)
{
   ...
   TypeRef sourcTypeRef = LLVMTypeOf(source);
   TypeRef ntype = get_base_type(to);

   unsigned sourceBits = LLVMGetIntTypeWidth(sourcTypeRef);
...
ValueRef create_bounds_check_function()
{
   ...
   TypeRef printfType = LLVMGlobalGetValuTypeRef(printfFunc);
   TypeRef exitType = LLVMGlobalGetValuTypeRef(exitFunc);
...
ValueRef safe_access_(Token *curr, Token *left, Token *right)
{
   ...
   if (size) {
      ...
      TypeRef funcType = LLVMGlobalGetValuTypeRef(checkFunc);
