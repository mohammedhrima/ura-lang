...
void print_inst(Inst *inst)
{
   ...
   curr->ir_reg ? debug("r%.4d:", curr->ir_reg) : debug("rxxxx:");
   int k = 0;
   ...
   case ASSIGN:
   {
      ...
      debug("r%.2d = r%.2d ", left->ir_reg, right->ir_reg);
   ...
   case AND: case OR:
   {
      debug("r%.2d to r%.2d ", left->ir_reg, right->ir_reg);
   ...
   case FCALL: debug("%s ", curr->name); break;
   ...
   case RETURN: debug("r%.2d ", left->ir_reg); break;
   ...
   case AS:
   {
      ...
      if (left->ir_reg) debug("r%.2d ", left->ir_reg);
      if (left->name) debug("(%s) ", left->name);
      debug("to %s", to_string(right->type));
      break;
   }
   case STACK:
   {
