...
Token *func_dec_ir(Node *node)
{
   ...
   for (int i = 0; params && i < node->left->cpos && !found_error; i++)
   {
      ...
      child->token->Param.ptr = node->token;
      ...
      if (check(!compatible(left, right), "type mismatch"))
      {
         ...
         while (ts < te)
         {
            ...
            case VALUE:
            {
               switch (token->type)
               {
               case INT: to_add += debug("%d", token->Int.value); break;
               case SHORT: to_add += debug("%d", token->Short.value); break;
               case LONG: to_add += debug("%lld", token->Long.value); break;
               case FLOAT: to_add += debug("%f", token->Float.value); break;
               case BOOL: to_add += debug("%s", token->Bool.value ? "True" : "False"); break;
               case CHARS: to_add += print_escaped(token->Chars.value); break;
               case CHAR: to_add += debug("%c", token->Char.value); break;
               default:
...
Token *generate_ir(Node *node)
{
   ...
   case BREAK: case CONTINUE:
   {
      ...
      bool found = false;
      for (int i = scoop_pos; i >= 0 && !found; i--)
      {
         Node *scoop = Gscoop[i];
...
void generate_asm(Inst *inst)
{
   ...
   case CHAR: case CHARS: case PTR: case FLOAT:
   {
      ...
      {
         // debug(" is_param");
         if (check(curr->Param.ptr == NULL, "error\n")) return;
         if (check(!curr->Param.ptr->llvm.is_set, "error\n")) return;
